...
public class GestureDetectGridView extends GridView {
        ...
        gDetector = new GestureDetector(context, new GestureDetector.SimpleOnGestureListener() {
            ...
            public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY){
                if(Math.abs(velocityX) + Math.abs(velocityY)> SWIPE_THRESHOLD_VELOCITY) {
                    mController.processSwipMovement(context, velocityX, velocityY);
                    return true;
                }
                else
                {
            ...
            public void onLongPress(MotionEvent event) {
                int position = GestureDetectGridView.this.pointToPosition
                        (Math.round(event.getX()), Math.round(event.getY()));
                mController.processLongPressMovement(context, position, true);
    ...
    public void setGame(Game Game, GameManager manager) {
        mController.setGame(Game);
        mController.setManager(manager);
    }

    public void setTimer(Timer timer, TextView timerText) {
        timer.setTimerText(timerText);
        mController.setTimer(timer);
    }
}
